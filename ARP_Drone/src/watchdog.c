...
int main() {
    ...
    const char *filename = "Logs/watchdog_log.txt";  
    bool brk = false;
   ...
   while (1) {
    ...
    for (int i = 0; i < MAX_PIDS; i++) {
        ...
        if (kill(target_pid, SIGUSR1) == 0) {
            ...
            FILE *log_file = fopen("Logs/watchdog_log.txt", "w");
            if (log_file != NULL) {
                fprintf(log_file, " Signal sent to process with PID %d.Process with PID %d is working.\n", target_pid, target_pid);
                fclose(log_file);
            } else {
            ...
            FILE *log_file = fopen("Logs/watchdog_log.txt", "a");
            if (log_file != NULL) {
